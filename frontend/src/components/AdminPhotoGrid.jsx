...
export default function AdminPhotoGrid({ slug, token, items }) {
  ...
  useBackToClose(isOpen, close) // tombol Back di HP menutup pratinjau, bukan meninggalkan halaman
        ...
        {items.map((p, i) => (
          <li key={p.drive_file_id}>
            <button
              ...
              aria-label={t('Perbesar {name}', { name: baseName(p.filename) })}
            >
              ...
              <span className="mt-1.5 flex flex-wrap items-center gap-x-1.5 gap-y-1 px-0.5">
                ...
                {p.note && <MessageSquare size={12} className="shrink-0 text-mute" aria-label={t('Ada catatan')} />}
                {p.is_extra && <span className="shrink-0 rounded-full bg-solid px-1.5 py-0.5 text-[10px] font-bold text-accent">{t('Tambahan')}</span>}
              </span>
            </button>
          </li>
        ))}
        ...
        <div role="dialog" aria-modal="true" aria-label={cur.filename} className="on-dark keep-light fixed inset-0 z-50 flex flex-col bg-solid text-onsolid animate-fade">
          <header className="flex h-14 shrink-0 items-center justify-between gap-3 px-4">
            <span className="min-w-0 truncate font-mono text-xs text-sand">
              ...
              {cur.is_extra && <span className="ml-2 rounded-full bg-ink2 px-2 py-0.5 font-sans text-[11px] font-bold text-accent">{t('Tambahan')}</span>}
            </span>
            <button type="button" onClick={close} aria-label={t('Tutup')} className="btn-ghost h-10 w-10 shrink-0 px-0">
              <X size={16} />
            </button>
          </header>
          <div className="relative flex min-h-0 flex-1 items-center justify-center px-4 sm:px-16">
            {items.length > 1 && (
              <button type="button" onClick={() => go(-1)} aria-label={t('Sebelumnya')} className="absolute left-2 top-1/2 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full bg-ink2 hover:bg-mute">
                <ChevronLeft size={18} />
              </button>
            )}
            ...
            {items.length > 1 && (
              <button type="button" onClick={() => go(1)} aria-label={t('Berikutnya')} className="absolute right-2 top-1/2 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full bg-ink2 hover:bg-mute">
          ...
          <footer className="shrink-0 px-4 pb-[max(16px,env(safe-area-inset-bottom))] pt-3">
            {cur.note ? (
              ...
              <p className="text-center text-xs text-faint">{t('Tidak ada catatan dari klien')}</p>
            )}
          </footer>
        </div>
      )}
