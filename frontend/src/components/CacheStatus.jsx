...
export default function CacheStatus({ sessionId, onReady, refreshKey = 0 }) {
  ...
  if (!st) return <span className="eyebrow animate-pulse">{t('Memeriksa cache')}</span>
  if (st.total === 0) return <span className="font-mono text-xs text-danger">{t('Folder tidak terbaca')}</span>
    ...
    <div className="font-mono text-xs">
      <div className="flex items-center gap-2">
        ...
        <span className={clsx(st.ready ? 'text-ink' : 'text-mute')}>
          {st.ready ? t('Galeri siap dibagikan') : t('Menyiapkan galeri… {a} / {b}', { a: st.thumb, b: st.total })}
        </span>
      </div>
      ...
      <p className="mt-1.5 text-[11px] text-mute">
        {t('thumbnail {a}% · foto besar {b}%', { a: pctThumb, b: pctFull })}
        {!allDone && !st.warming && t(' · sisanya dimuat saat dibuka')}
      </p>
    </div>
  )
}
