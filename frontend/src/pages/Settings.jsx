...
import { DEFAULT_WA_TEMPLATE, PRESETS, TEXT_SCALES, WA_PLACEHOLDERS, applyTheme, themeVars, fillWaTemplate, loadFonts, setBranding, useStudio } from '../utils/theme'
...
export default function Settings() {
  ...
  const saveTheme = async () => {
    try {
      ...
      setToast(t('Tersimpan — klien langsung melihat tampilan baru'))
    } catch (e) {
  ...
  return (
    <AdminShell eyebrow={t('Pengaturan')} title={t(SETTINGS_TABS.find(([id]) => id === tab)[1])}>
      <Toast message={toast} onClose={() => setToast('')} />

      <div className="grid grid-cols-[minmax(0,1fr)] gap-6">
        {/* On desktop the sub-topics live in the sidebar under "Pengaturan"; phones get this row */}
        <nav aria-label={t('Bagian pengaturan')} className="lg:hidden">
        ...
        <div className="min-w-0">
          ...
          {tab !== 'identitas' && !theme && <p className="eyebrow animate-pulse">{t('Memuat')}</p>}
      ...
      {theme && dirty && ['tampilan', 'galeri', 'pesan'].includes(tab) && (
        <div className="sticky bottom-3 z-20 mt-10 flex items-center gap-2 rounded-[22px] bg-solid p-2 pl-4 text-onsolid shadow-lg">
          <span className="min-w-0 truncate text-sm text-sand">{t('Belum disimpan')}</span>
  ...
  return (
    <div className="grid gap-10 lg:grid-cols-[minmax(0,30rem)_1fr]">
      <div className="space-y-10">
        <section>
          <p className="label">{t('Tema warna siap pakai')}</p>
          <div className="grid grid-cols-3 gap-2 sm:grid-cols-6 lg:grid-cols-3">
            {PRESETS.map((p) => (
              ...
              >
                ...
                <span className="mt-1.5 block font-semibold">{t(p.name)}</span>
              </button>
            ))}
          </div>
        </section>
        ...
        <section>
          <p className="label">{t('Atau pilih warna sendiri')}</p>
          ...
          <p className="mt-2 text-xs text-mute">{t('Warna turunan (abu-abu, garis, teks di atas tombol) dihitung otomatis supaya tetap mudah dibaca.')}</p>
        </section>

        <section>
          <p className="label">{t('Mode warna untuk klien')}</p>
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              >
                ...
                <span className="block text-xs text-mute">{t(desc)}</span>
              </button>
            ))}
          </div>
          <p className="mt-2 text-xs text-mute">{t('Versi gelap dibuat otomatis dari warna yang sama: warna teks jadi latar, warna latar jadi teks, aksen tetap. Intro dan tampilan foto besar selalu gelap.')}</p>
        </section>

        <section>
          <p className="label">{t('Ukuran teks di galeri klien')}</p>
        ...
        <section>
          <p className="label">{t('Font judul')}</p>
          ...
          <label className="mt-3 flex items-center gap-2 text-sm">
            ...
            {t('Judul miring (italic)')}
          </label>
        </section>

        <section>
          <p className="label">{t('Font teks')}</p>
          <div className="grid grid-cols-2 gap-2">
  ...
  return (
    ...
    >
      <span className="truncate text-xl leading-tight" style={{ fontFamily: `'${font}'`, fontStyle: display && italic ? 'italic' : 'normal' }}>
        {display ? 'Ayu & Bagus' : t('Pilih foto favorit')}
      </span>
  ...
  return (
    <div className="lg:sticky lg:top-6 lg:self-start">
      <div className="mb-3 flex items-center justify-between gap-2">
        <p className="eyebrow">{t('Pratinjau galeri klien')}</p>
      ...
      <div className="overflow-hidden rounded-[26px] border border-line bg-paper p-5 text-ink" style={{ fontSize: `${theme.text_scale || 1}rem`, ...themeVars(theme, dark) }}>
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          <span className="rounded-full bg-solid px-3 py-1 text-[0.8em] text-onsolid">{t('Pilih sebelum {date}', { date: t('Jumat, 25 Sep') })}</span>
          <span className="rounded-full bg-accent px-3 py-1 text-[0.8em] font-bold text-onaccent">{t('{n} hari lagi', { n: 7 })}</span>
        </div>
        ...
        <div className="mt-5 flex items-center gap-3 rounded-[22px] bg-solid p-3 text-onsolid">
          ...
          <span className="text-[0.75em] text-sand">{t('Sisa {n} foto dalam paket', { n: 28 })}</span>
          <span className="btn-accent ml-auto h-10 px-4 text-[0.85em]">{t('Kirim')}</span>
        </div>
      </div>
    </div>
  )
  ...
  return (
    ...
    <div className="grid gap-8">
      <section className="rounded-[22px] bg-card p-5">
        <p className="label">{t('Kemudahan untuk klien')}</p>
        <p className="mb-4 text-sm text-mute">{t('Untuk klien yang kurang terbiasa dengan aplikasi (misalnya orang tua). Ukuran teks ada di Tampilan.')}</p>
        <Toggle
          checked={theme.simple_mode}
          onChange={(v) => up({ simple_mode: v })}
          title={t('Mode sederhana')}
          desc={t('Menyembunyikan fitur “Tandai dulu” dan membuat tombol panduan lebih besar, supaya pilihan di layar lebih sedikit.')}
        />
      </section>

      <section className="rounded-[22px] bg-card p-5">
        <p className="label">{t('Intro pembuka')}</p>
        <Toggle checked={theme.intro_enabled} onChange={(v) => up({ intro_enabled: v })} title={t('Tampilkan intro')} desc={t('Muncul sekali saat klien membuka link. Klien selalu bisa mengetuk untuk melewati.')} />
        {theme.intro_enabled && (
          <div className="mt-5 grid grid-cols-2 gap-2 sm:grid-cols-4">
            {INTRO_STYLES.map(([id, label, desc]) => (
              ...
              >
                <span className="block text-sm font-semibold">{t(label)}</span>
                <span className="mt-0.5 block text-xs text-mute">{t(desc)}</span>
              </button>
            ))}
          </div>
        )}
        ...
        {theme.intro_enabled && theme.intro_style !== 'video' && (
          <div className="mt-5 grid gap-5 sm:grid-cols-2">
            <div>
              <label className="label" htmlFor="intro-text">{t('Teks sebelum nama klien')}</label>
              <input autoComplete="off" id="intro-text" className="field" value={theme.intro_text} onChange={(e) => up({ intro_text: e.target.value })} placeholder={t('Galeri untuk')} maxLength={60} />
            </div>
            <div>
              <label className="label" htmlFor="intro-sec">{t('Lama tampil: {n} detik', { n: Number(theme.intro_seconds).toFixed(1) })}</label>
      ...
      <section className="rounded-[22px] bg-card p-5">
        <p className="label">{t('Galeri')}</p>
        <label className="label mt-2" htmlFor="gtitle">{t('Judul kecil di atas nama klien')}</label>
        ...
        <Toggle className="mt-5" checked={theme.guide_enabled} onChange={(v) => up({ guide_enabled: v })} title={t('Panduan otomatis')} desc={t('Panduan cara memilih muncul sekali saat klien pertama membuka. Tombol “Cara memilih” selalu tersedia.')} />
      </section>
    </div>
...
function IntroVideo() {
  ...
  const upload = async (variant, file) => {
    if (!file) return
    setMsg('')
    if (!['video/mp4', 'video/webm'].includes(file.type)) return setMsg(t('Format harus MP4 (disarankan) atau WebM.'))
    if (file.size > VIDEO_RULES.maxMB * 1024 * 1024) return setMsg(t('Ukuran {a} MB — maksimal {b} MB. Kecilkan ke 720p atau perpendek durasinya.', { a: (file.size / 1048576).toFixed(1), b: VIDEO_RULES.maxMB }))
    const meta = await readVideoMeta(file)
    if (!meta) return setMsg(t('Video tidak bisa dibaca browser. Ekspor ulang sebagai MP4 (H.264).'))
    if (meta.seconds > VIDEO_RULES.maxSeconds) return setMsg(t('Durasi {a} detik — maksimal {b} detik.', { a: meta.seconds.toFixed(1), b: VIDEO_RULES.maxSeconds }))
    const notes = []
    if (meta.seconds > VIDEO_RULES.warnSeconds) notes.push(t('durasi {a} dtk (ideal ≤ {b})', { a: meta.seconds.toFixed(1), b: VIDEO_RULES.warnSeconds }))
    if (Math.max(meta.width, meta.height) > 1920) notes.push(t('resolusi di atas 1080p tidak perlu'))
    if (variant === 'mobile' && meta.width > meta.height) notes.push(t('versi HP sebaiknya tegak (9:16)'))
    setBusy(variant)
    setProgress(0)
    try {
      const b = await adminApi.uploadIntroVideo(variant, file, setProgress)
      setBranding({ ...b, theme: studio?.theme || b.theme })
      setMsg(t('Video terunggah ({a} MB, {b} dtk).', { a: (file.size / 1048576).toFixed(1), b: meta.seconds.toFixed(1) }) + (notes.length ? t(' Catatan: ') + notes.join('; ') + '.' : ''))
    } catch (e) {
  ...
  const remove = async (variant) => {
    const ok = await ask({
      title: t('Hapus video intro ini?'),
      message: t('Klien akan kembali melihat animasi logo bawaan. Anda bisa mengunggah video lagi kapan saja.'),
  ...
  return (
    <div className="mt-5 space-y-4">
          ...
          <div key={variant} className="rounded-2xl border border-line p-3">
            ...
            <div className="mt-3 flex gap-2">
              <button type="button" className="btn-ghost h-9 px-3 text-xs" disabled={!!busy} onClick={() => refs[variant].current?.click()}>
                {busy === variant ? t('Mengunggah {n}%', { n: Math.round(progress * 100) }) : vids[variant] ? t('Ganti') : t('Unggah')}
              </button>
              {vids[variant] && (
                <button type="button" className="btn-ghost h-9 w-9 px-0 text-danger" aria-label={t('Hapus video')} onClick={() => remove(variant)}>
                  <Trash2 size={13} />
                </button>
              )}
            </div>
      ...
      <div className="rounded-2xl bg-paper p-4 text-xs leading-relaxed text-mute">
        <p className="mb-1 font-semibold text-ink">{t('Ketentuan video intro')}</p>
        <ul className="list-disc space-y-0.5 pl-4">
          <li><b className="text-ink">{t('Durasi 3–6 detik')}</b> {t('(maks {n}). Klien datang untuk melihat foto, bukan menonton.', { n: VIDEO_RULES.maxSeconds })}</li>
          <li><b className="text-ink">{t('Ukuran maks {n} MB', { n: VIDEO_RULES.maxMB })}</b>{t(', idealnya di bawah 3 MB, supaya cepat di data seluler.')}</li>
          <li><b className="text-ink">MP4 (H.264), 720p</b> {t('sudah cukup tajam di HP. Tidak perlu 4K.')}</li>
          <li><b className="text-ink">{t('Tanpa suara.')}</b> {t('Browser HP memutar video otomatis hanya jika dibisukan.')}</li>
          <li>{t('Versi HP (tegak 9:16) opsional. Tanpa itu, versi utama ditampilkan utuh dengan latar gelap.')}</li>
          <li>{t('Jika video belum siap dalam 2,5 detik (sinyal lemah), otomatis diganti animasi logo supaya klien tidak menunggu.')}</li>
        </ul>
      </div>
      {confirmDialog}
    </div>
...
function MessageSettings({ theme, up, studio }) {
  ...
  const isDefault = theme.wa_template.trim() === DEFAULT_WA_TEMPLATE.trim()
  const sample = fillWaTemplate(isDefault ? t(DEFAULT_WA_TEMPLATE) : theme.wa_template, {
    nama: 'Ayu & Bagus',
    link: 'https://studio-kamu.com/g/Ab12Cd34',
    pin: '4827',
    paket: 40,
    tambahan: isDefault ? t(' (kalau mau lebih, bisa sampai {n} dengan biaya tambahan)', { n: 50 }) : ' (kalau mau lebih, bisa sampai 50 dengan biaya tambahan)',
    deadline: isDefault ? t('Jumat, 25 September') : 'Jumat, 25 September',
    studio: studio?.studio_name || t('Nama Studio'),
  })
  const insert = (tag) => {
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-2">
      <div>
        <label className="label" htmlFor="wa">{t('Template pesan')}</label>
        ...
        <p className="mt-2 text-xs text-mute">
          {isDefault
            ? t('Ini template bawaan: pesan otomatis mengikuti bahasa panel (Indonesia / Inggris). Setelah diubah, teks Anda dipakai apa adanya.')
            : t('Template buatan Anda dipakai apa adanya, apa pun bahasa panelnya.')}{' '}
          {!isDefault && (
            <button type="button" className="underline hover:text-ink" onClick={() => up({ wa_template: DEFAULT_WA_TEMPLATE })}>
              {t('Kembalikan ke template bawaan')}
            </button>
          )}
        </p>
        <p className="mb-2 mt-4 text-xs text-mute">{t('Klik untuk menyisipkan data otomatis:')}</p>
        <div className="flex flex-wrap gap-1.5">
          {WA_PLACEHOLDERS.map(([tag, desc]) => (
            <button key={tag} type="button" onClick={() => insert(tag)} title={t(desc)} className="rounded-full border border-line bg-card px-2.5 py-1 font-mono text-xs hover:border-ink">
              {tag}
            </button>
          ))}
        </div>
        <ul className="mt-3 space-y-0.5 text-xs text-mute">
          {WA_PLACEHOLDERS.map(([tag, desc]) => (
            <li key={tag}>
              <span className="font-mono text-ink">{tag}</span> — {t(desc)}
            </li>
          ))}
        </ul>
      </div>
      <div>
        <p className="label">{t('Contoh hasilnya')}</p>
        <div className="rounded-2xl bg-[#E7FFDB] p-4 text-sm leading-relaxed text-[#111B21] shadow-sm" style={{ whiteSpace: 'pre-wrap' }}>
          {sample}
        </div>
        <p className="mt-2 text-xs text-mute">{t('Hindari emoji: sebagian HP menampilkannya sebagai “�” lewat tombol WhatsApp.')}</p>
      </div>
    </div>
  )
}
...
function DomainSecurity({ adm, setAdm, onToast }) {
  ...
  const saveUrl = async () => {
    ...
    try {
      ...
      onToast(url ? t('Alamat publik disimpan') : t('Alamat publik dikosongkan — memakai FRONTEND_URL dari .env'))
    } catch (e) {
  ...
  const savePw = async (e) => {
    e.preventDefault()
    if (pw.next !== pw.again) return onToast(t('Password baru dan ulangannya tidak sama'))
    setBusy(true)
    try {
      await adminApi.changePassword(pw.current, pw.next)
      setPw({ current: '', next: '', again: '' })
      setAdm({ ...adm, password_from_panel: true })
      onToast(t('Password admin diganti'))
    } catch (err) {
  ...
  return (
    <div className="grid max-w-2xl gap-8">
      <section className="rounded-[22px] bg-card p-5">
        <p className="label">{t('Alamat publik (domain)')}</p>
        <p className="mb-4 text-sm text-mute">
          {t('Alamat yang dipakai di link galeri & pesan WhatsApp. Isi dengan domain studio (mis.')} <span className="font-mono">https://pilih.studiokamu.com</span>{t(') atau alamat ngrok. Kosongkan untuk memakai pengaturan server.')}
        </p>
        <div className="flex flex-wrap gap-2">
        ...
        <button type="button" className="mt-3 text-xs text-mute underline hover:text-ink" onClick={() => setUrl(window.location.origin)}>
          {t('Pakai alamat yang sedang dibuka ({url})', { url: window.location.origin })}
        </button>
        <p className="mt-4 text-xs text-mute">
          {t('Catatan: mengarahkan domain ke server (DNS & hosting) dilakukan di penyedia domain/hosting, bukan dari sini. Lihat README bagian “Hosting”.')}
        </p>
      </section>

      <form onSubmit={savePw} className="rounded-[22px] bg-card p-5">
        <p className="label">{t('Ganti password admin')}</p>
        <p className="mb-4 text-sm text-mute">
          {adm.password_from_panel ? t('Password saat ini diatur dari panel ini.') : t('Saat ini memakai ADMIN_PASSWORD dari file .env. Setelah diganti di sini, password baru yang berlaku.')}
        </p>
        <div className="grid gap-4 sm:grid-cols-3">
          <input type="password" autoComplete="current-password" className="field" placeholder={t('Password lama')} value={pw.current} onChange={(e) => setPw({ ...pw, current: e.target.value })} />
          <input type="password" autoComplete="new-password" className="field" placeholder={t('Password baru (min. 8)')} value={pw.next} onChange={(e) => setPw({ ...pw, next: e.target.value })} />
          <input type="password" autoComplete="new-password" className="field" placeholder={t('Ulangi password baru')} value={pw.again} onChange={(e) => setPw({ ...pw, again: e.target.value })} />
        </div>
...
function Identity({ onToast }) {
  ...
  const save = async (e) => {
    ...
    try {
      ...
      onToast(t('Identitas studio disimpan'))
    } catch (err) {
      onToast(errorMessage(err))
    } finally {
      setBusy(false)
  ...
  const removeLogo = async () => {
    const ok = await ask({
      title: t('Hapus logo studio?'),
      message: t('Galeri klien akan memakai nama studio saja. Anda bisa mengunggah logo lagi kapan saja.'),
      confirmLabel: t('Ya, hapus'),
      danger: true,
    })
    if (!ok) return
    try {
      ...
      onToast(t('Logo dihapus'))
    } catch (err) {
      onToast(errorMessage(err))
    }
  }

  if (!b) return <p className="eyebrow animate-pulse">{t('Memuat')}</p>
  ...
  return (
    <div className="grid gap-12 md:grid-cols-[minmax(0,28rem)_1fr]">
      <form onSubmit={save} className="space-y-8">
        <div>
          <label className="label" htmlFor="name">{t('Nama studio')}</label>
          <input autoComplete="off" id="name" className="field text-lg" value={form.studio_name} onChange={set('studio_name')} placeholder={t('Nama studio kamu')} maxLength={120} />
          <p className="mt-2 text-xs text-mute">{t('Tampil di intro, galeri klien, halaman PIN, dan judul dashboard.')}</p>
        </div>
        <div>
        ...
        <div>
          <label className="label" htmlFor="contact">{t('Kontak')}</label>
          ...
          <p className="mt-2 text-xs text-mute">{t('Ditampilkan di footer galeri agar klien tahu ke mana bertanya.')}</p>
        </div>
        <div>
          <label className="label" htmlFor="wa-number">{t('Nomor WhatsApp studio')}</label>
          ...
          <p className="mt-2 text-xs text-mute">{t('Memunculkan tombol “Chat fotografer” di footer galeri klien. Boleh ditulis 0812… atau +62812…, nanti dirapikan otomatis. Kosongkan untuk menyembunyikan tombolnya.')}</p>
        </div>
        <div>
          ...
          <div className="flex items-center gap-3">
            {b.logo_url ? (
              ...
              <span className="flex h-14 w-28 items-center justify-center rounded-xl border border-dashed border-line text-[11px] text-faint">{t('tanpa logo')}</span>
            )}
            ...
            <button type="button" className="btn-ghost h-10 px-4 text-xs" onClick={() => fileRef.current?.click()}>
              <ImagePlus size={14} /> {b.logo_url ? t('Ganti') : t('Unggah')}
            </button>
            {b.logo_url && (
              <button type="button" className="btn-ghost h-10 w-10 px-0 text-danger" aria-label={t('Hapus logo')} onClick={removeLogo}>
                <Trash2 size={14} />
              </button>
            )}
          </div>
          <p className="mt-2 text-xs text-mute">{t('PNG/SVG berlatar transparan, maks 2 MB.')}</p>
        </div>
        <button type="submit" className="btn-ink" disabled={busy}>
          {busy ? t('Menyimpan…') : t('Simpan identitas')}
        </button>
      </form>

      <div>
        <p className="eyebrow mb-3">{t('Pratinjau di galeri klien')}</p>
        <div className="rounded-[26px] border border-line bg-paper p-6">
          <BrandHeader branding={preview} />
          <p className="eyebrow mt-8">{t('Pilih foto favorit Anda')}</p>
          <p className="mt-2 font-display text-4xl">{t('Nama Klien')}</p>
...
function IntroPreview({ theme }) {
  ...
  const name = studio?.studio_name || t('Nama Studio')
  ...
  return (
    <div className="xl:sticky xl:top-6 xl:self-start">
      <div className="mb-3 flex items-center justify-between">
        <p className="eyebrow">{t('Pratinjau di HP')}</p>
      ...
      <div className="relative mx-auto h-[500px] w-[250px] overflow-hidden rounded-[34px] border-[6px] border-ink bg-paper shadow-xl">
        ...
        <div className="absolute inset-0 overflow-hidden px-3 pt-4" style={{ fontSize: `${10 * scale}px` }}>
          <div className="flex items-center justify-between">
            ...
            <span className={clsx('rounded-full', theme.simple_mode ? 'bg-solid px-2 py-1 text-[0.9em] text-onsolid' : 'border border-line px-1.5 py-0.5 text-[0.8em]')}>? {t('Cara memilih')}</span>
          </div>
          ...
          <div className="absolute inset-x-2 bottom-2 flex items-center gap-2 rounded-2xl bg-solid px-2.5 py-2 text-onsolid">
            ...
            <span className="ml-auto rounded-full bg-accent px-2.5 py-1 text-[0.9em] font-bold text-onaccent">{t('Kirim')}</span>
          </div>
        </div>
          ...
          >
            {style === 'video' ? (
              video ? (
                ...
                <p className="text-center text-xs text-sand">{t('Belum ada video. Unggah di bawah — sampai ada, gaya “Logo → pojok” yang dipakai.')}</p>
              )
            ) : style === 'morph' ? (
              <>
                <img
      ...
      <p className="mt-3 text-center text-xs text-mute">{t('Pratinjau ikut berubah saat pengaturan di kiri diubah.')}</p>
    </div>
  )
}
